...
aditmadzs = LineClient(authToken='ExFSLcAAKLheIv2ao0e8.yAM1ZO3J61gOwuemMdtLYa.5Jx3UJQE9F0yXAfKpyuL1bb7AU9xAHBGJlo8d4MsPtQ=')
...
ki = LineClient(authToken='Ex5roIy4AsGz4gVjmAid.mKZbV5HoQB+/lk9EZno5+q.OzH6bden9xISnuHmi6XUM3NjSGxRiOMwD5pGqoz+Muo=')
ki.log("Auth Token : " + str(ki.authToken))
channel1 = LineChannel(ki)
ki.log("Channel Access Token : " + str(channel1.channelAccessToken))

kk = LineClient(authToken='Ex32LteN9OQ690MzWhw8.9b6szpiJ3KHyLZhfedasIa.QZXL4auhpAyckpOwzJUZ/8W16fP9m2wiqZI0vZCLGL4=')
kk.log("Auth Token : " + str(kk.authToken))
channel2 = LineChannel(kk)
kk.log("Channel Access Token : " + str(channel2.channelAccessToken))

kc = LineClient(authToken='ExtnH9ldJKzVLUZIc9T5.fj0S/qYKVWGRwpehA8QPbq.9mWGRsTlyKlXITec/ZHViVazrmy2zYEZEerd+Dgdbdw=')
...
#===========BOT UPDATE============#
                        ...
                        elif cmd.startswith("sepri1name: "):
                          if msg._from in admin:
                            separate = msg.text.split(" ")
                            string = msg.text.replace(separate[0] + " ","")
                            if len(string) <= 10000000000:
                        ...
                        elif cmd.startswith("sepri2name: "):
                          if msg._from in admin:
                            separate = msg.text.split(" ")
                            string = msg.text.replace(separate[0] + " ","")
                            if len(string) <= 10000000000:
                        ...
                        elif cmd.startswith("sepri3name: "):
                          if msg._from in admin:
                            separate = msg.text.split(" ")
                            string = msg.text.replace(separate[0] + " ","")
                            if len(string) <= 10000000000:
                          ...
                          if wait["selfbot"] == True:
                              if settings["autoJoinTicket"] == True:
                                 ...
                                 for ticket_id in n_links:
                                     ...
                                     aditmadzs.sendMessage(msg.to, "JOIN GROUP : %s" % str(group.name))
                                     group1 = ki.findGroupByTicket(ticket_id)
                                     ki.acceptGroupInvitationByTicket(group1.id,ticket_id)
                                     ki.sendMessage(msg.to, "JOIN GROUP : %s" % str(group.name))
                                     group2 = kk.findGroupByTicket(ticket_id)
                                     kk.acceptGroupInvitationByTicket(group2.id,ticket_id)
                                     kk.sendMessage(msg.to, "JOIN GROUP : %s" % str(group.name))
                                     group3 = kc.findGroupByTicket(ticket_id)
                                     kc.acceptGroupInvitationByTicket(group3.id,ticket_id)
                                     kc.sendMessage(msg.to, "JOIN GROUP : %s" % str(group.name))
